...
public class ReadabilityStatistics {
    ...
    private static final List<String> ADD_SYLLABLES = Collections.unmodifiableList(Arrays.asList("ia", "riet", "dien", "iu", "io", "ii", "[aeiouym]bl$", "[aeiou]{3}", "^mc", "ism$", "([^aeiouy])\\1l$", "[^l]lien", "^coa[dglx].", "[^gq]ua[^auieo]", "dnt$", "uity$", "ie(r|st)$"));

    // Single syllable prefixes and suffixes
    private static final List<String> PREFIX_SUFFIX = Collections.unmodifiableList(Arrays.asList("^un", "^fore", "ly$", "less$", "ful$", "ers?$", "ings?$"));
    ...
    public static double daleChallScore(String strText) {
        ...
        for (int i = 0; i < intWordCount; ++i) {
            if (!DALECHALL_WORDLIST.contains(arrWords.get(i))) {
    ...
    private static int syllableCount(String strWord) {
        ...
        for(String regex : PREFIX_SUFFIX) {
        ...
        for(String strSyllable : SUB_SYLLABLES) {
            intSyllableCount -= PHPMethods.preg_match(strSyllable, strWord);
        }
        for(String strSyllable : ADD_SYLLABLES) {
            intSyllableCount += PHPMethods.preg_match(strSyllable, strWord);
        }
